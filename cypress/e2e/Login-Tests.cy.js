...
describe('UI Authentication Tests', function () {
  ...
  it('should validate login form fields', function () {
    ...
    cy.get('.error-messages').should('be.visible').and('contain', 'email or password is invalid')
  ...
  it('should verify login page UI elements', function () {
    ...
    // Verify form fields
    login.email().should('have.attr', 'type', 'email').and('have.attr', 'placeholder', 'Email')
    ...
    // Verify button state
    login.signInButton().should('be.visible').and('be.enabled').and('contain', 'Sign in')
