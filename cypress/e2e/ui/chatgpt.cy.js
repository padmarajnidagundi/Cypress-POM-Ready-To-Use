...
describe('ChatGPT UI Tests', () => {
  ...
  it('should send a message and receive a response', () => {
    ...
    chatGpt.getLastResponse().should('be.visible').and('not.be.empty')
  })
  ...
  it('should maintain chat history', () => {
    ...
    chatGpt.getChatHistory().should('contain', messages[0]).and('contain', messages[1])
  })
  ...
  it('should open menu and settings', () => {
    ...
    chatGpt.getElement(chatGpt.selectors.menuButton).should('be.visible').click()

    // Verify settings button is visible in menu
    chatGpt.getElement(chatGpt.selectors.settingsButton).should('be.visible').click()
  ...
  it('should verify complete message interaction flow', () => {
    ...
    chatGpt.sendMessage(testMessage)
    chatGpt.getElement(chatGpt.selectors.loadingIndicator).should('be.visible')

    // Wait for and verify response
    chatGpt.waitForResponse()

    // Verify message appears in both user and assistant sections
    chatGpt.getUserMessages().should('contain', testMessage)
    chatGpt.getAssistantMessages().should('be.visible').and('not.be.empty')

    // Verify regenerate button is available
    chatGpt.getElement(chatGpt.selectors.regenerateButton).should('be.visible').and('be.enabled')
    ...
    chatGpt.getChatHistory().should('not.contain', testMessage)
    chatGpt.getElement(chatGpt.selectors.messageInput).should('be.empty')
  })
  ...
  describe('Error Handling', () => {
    ...
    it('should handle network timeout gracefully', () => {
      ...
      chatGpt.getErrorMessage().should('be.visible').and('contain', 'timeout')

      // Verify UI remains functional
      chatGpt.getElement(chatGpt.selectors.messageInput).should('be.enabled')
      chatGpt.getElement(chatGpt.selectors.sendButton).should('be.enabled')
    })
    ...
    it('should handle server errors appropriately', () => {
      ...
      chatGpt.getErrorMessage().should('be.visible').and('contain', 'error')

      // Verify retry functionality
      chatGpt
        .getElement(chatGpt.selectors.regenerateButton)
        .should('be.visible')
        .and('be.enabled')
        .click()
    ...
    it('should handle rate limiting', () => {
      ...
      chatGpt.getErrorMessage().should('be.visible').and('contain', 'rate limit')

      // Verify cool-down message
      cy.contains('Please wait before sending more messages').should('be.visible')

      // Verify message input is temporarily disabled
      chatGpt.getElement(chatGpt.selectors.messageInput).should('be.disabled')
    })
  })
  ...
  it('example: should accept pasted text into input', () => {
    ...
    chatGpt.getElement(chatGpt.selectors.messageInput).invoke('val', pasted).trigger('input')
    chatGpt.getElement(chatGpt.selectors.sendButton).click()
    chatGpt.waitForResponse()
