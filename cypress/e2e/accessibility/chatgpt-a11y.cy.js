...
describe('ChatGPT Accessibility Tests', () => {
  ...
  it('should have proper keyboard navigation', () => {
    ...
    cy.get(chatGpt.selectors.menuButton).focus().type('{enter}')
  ...
  it('should verify ARIA attributes on interactive elements', () => {
    ...
    chatGpt
      .checkElementAccessibility(chatGpt.selectors.messageInput, { tabIndex: '0' })
      .should('have.attr', 'aria-placeholder')
      .and('have.attr', 'aria-multiline', 'true')

    // Check send button accessibility
    chatGpt
      .checkElementAccessibility(chatGpt.selectors.sendButton)
      .should('have.attr', 'aria-pressed', 'false')
      .and('have.attr', 'role', 'button')

    // Check menu button accessibility
    chatGpt
      .checkElementAccessibility(chatGpt.selectors.menuButton)
  ...
  it('should verify focus management in chat interface', () => {
    ...
    chatGpt.waitForResponse()
    chatGpt.checkFocusableElement(chatGpt.selectors.regenerateButton).focus()
  ...
  it('should verify dynamic ARIA updates', () => {
    ...
    cy.get('[role="status"]').should('exist').and('have.attr', 'aria-live', 'polite')

    // Verify response containers
    chatGpt.waitForResponse()
    cy.get('[role="log"]').should('exist').and('have.attr', 'aria-label').and('not.be.empty')
  })
