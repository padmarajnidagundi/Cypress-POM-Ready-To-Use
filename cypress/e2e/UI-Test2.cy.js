...
describe('UI Test Suite - Cypress Querying Examples', () => {
  ...
  it('should verify the querying examples buttons', () => {
    ...
    homePage.getQueryingExamples().each((button) => {
      cy.wrap(button).should('be.visible').and('have.attr', 'class').and('include', 'query-btn')
    })
  })
  ...
  it('should demonstrate DOM traversal methods', () => {
    ...
    cy.get('.query-button').parent().should('have.class', 'query-container')

    cy.get('.query-list').children().should('have.length.gt', 0)

    cy.get('.query-button').siblings().should('exist')
  })
  ...
  it('should verify element states and properties', () => {
    ...
    homePage.getQueryingExamples().first().should('be.visible').and('not.be.disabled')

    cy.get('.query-button').should('have.attr', 'type').and('not.be.empty')

    cy.get('.query-btn').should('have.class', 'query-btn').and('not.have.class', 'disabled')
  })
  ...
  it('should interact with form elements', () => {
    ...
    cy.get('.query-form').find('input').type('test query').should('have.value', 'test query')

    cy.get('.query-form').submit().should('have.class', 'submitted')
  })
  ...
  it('should handle dynamic content', () => {
    ...
    cy.get('.dynamic-content').invoke('text').should('not.be.empty')
  })
  ...
  it('should verify error states', () => {
    ...
    cy.get('.error-state').should('have.class', 'error')
  })
  ...
  it('should verify accessibility attributes', () => {
    ...
    cy.get('.query-button').should('have.attr', 'aria-label')

    cy.get('.query-form').find('input').should('have.attr', 'tabindex').and('not.eq', '-1')

    cy.get('.query-button').first().focus().should('have.focus')
  })
  ...
  describe('Advanced Accessibility Tests', () => {
    ...
    it('should verify color contrast ratios', () => {
      cy.get('.query-button').each(($button) => {
        cy.wrap($button).should('have.css', 'background-color').and('have.css', 'color')
      })
    })
  })
